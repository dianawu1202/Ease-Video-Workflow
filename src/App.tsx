...
export default function App() {
  ...
  const [referencePickTargetVideoId, setReferencePickTargetVideoId] = useState<string | null>(null);
  ...
  const handleStartReferencePick = React.useCallback((videoNodeId: string) => {
    ...
    closeHistoryPanel();
    closeAssetLibrary();
  }, [closeAssetLibrary, closeHistoryPanel, closeWorkflowPanel, setSelectedNodeIds]);
  ...
  const handlePickReferenceNode = React.useCallback((sourceNode: NodeData) => {
    ...
    if (canConnect) {
      setNodes(prev => prev.map(node => {
        if (node.id !== referencePickTargetVideoId) return node;
