...
export const useConnectionDragging = () => {
    ...
    const completeConnectionDrag = (
        onAddNext: (nodeId: string, direction: 'left' | 'right', position?: { x: number; y: number }) => void,
        ...
        // Drop on blank canvas - open Add Nodes menu at the release point.
        if (!hoveredNodeId) {
            onAddNext(
                connectionStart.nodeId,
                connectionStart.handle,
                tempConnectionEnd || { x: window.innerWidth / 2, y: window.innerHeight / 2 }
            );
        }
